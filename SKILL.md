...
| `lan-file-sharing` | Catalogued workflow. Run pinned Dufs with its complete native CLI/config behavior, use Python `http.server` as an explicit limited fallback for simple temporary downloads, and optionally expose an approved local HTTP port through user-managed ngrok. |
