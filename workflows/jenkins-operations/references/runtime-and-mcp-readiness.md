...
分别报告三层结果与未完成项。仅 Skill 文件存在、独立 MCP 客户端握手成功，或 Agent 能回答自身身份，都不能宣称目标 Agent 的 Jenkins 能力已就绪。`specialized-agent-deployment` 负责安排和汇总这些检查，各宿主适配器负责接入协议；Jenkins 操作和服务实现始终归本工作流。
