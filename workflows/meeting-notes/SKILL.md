...
`scripts/meeting_notes.py` 由宿主命令工具调用，须保留同目录的转写、语音、运行时和笔记写入模块。编排解释器、FunASR 解释器、VoxCPM 解释器是不同角色，安装依赖或验证导入时必须使用实际承担该任务的解释器；版本与哈希以对应 capability、安装指引及锁文件为准，不在部署端另列版本。

转写需要 FFmpeg、FunASR 运行时和完整模型资产；语音播报才需要 VoxCPM、其模型与 CUDA 条件，不使用播报时不要求安装。Obsidian 输出通过本地 Vault 文件写入，不依赖 Obsidian MCP；目标 Agent 仍须能访问用户指定 Vault。首次使用或环境变化时先检查脚本 `--help`，再分别运行所需 doctor：当前 `doctor --with-voice` 走语音专用检查并提前返回，不能替代普通转写 doctor；两种能力都需要时分别检查。模型预检不等于真实音频转写或播报已成功。
...
8. 仅在用户明确请求音频时使用语音分支：
   ...
   长文本会在 worker 内按自然标点无损切块、只加载一次模型后按序合成并合并为一个 WAV；`speak` 默认总超时至少为 900 秒，特别长的文本仍按长度增加，涵盖模型加载、全部分块推理和音频写入。可用 `--timeout-seconds <秒>` 显式覆盖；完成后立即返回，不固定等待到超时上限。超时返回 `processing-failed` 并清理本次新输出。
