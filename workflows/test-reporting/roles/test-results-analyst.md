...
- local purpose: 将用户明确提供的已有测试材料整理为规范 Markdown 测试报告，如实标注缺失、冲突与不可读，不扩大 Jenkins 或 Git 权限。
...
- 输出规范 Markdown 测试报告到 `workflows/test-reporting/outputs/` 下当前 report/run 目录或用户指定路径；Jenkins/JUnit 是可选材料，构建状态与测试结论必须分离。
