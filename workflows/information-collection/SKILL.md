...
本工作流由宿主 Agent 编排，没有必须部署的统一 MCP 或采集服务器；`scripts/routing.py` 只是 Python 标准库实现的路由状态逻辑，不负责真实抓取、认证或安装。实际依赖由所选路线决定：原生网页工具/HTTP 客户端、获准使用的 Tavily，或浏览器自动化。Tavily 的 Skill 文本不包含其 CLI/脚本运行时和认证；浏览器连接工具存在也不代表浏览器内核与所需登录会话可用。

首次使用或迁移后，在目标 Agent 内确认所选工具真实可调用，并读取一个任务范围内的来源，验证得到的是正文而非登录页或错误页。未选用的 Tavily、浏览器、文档转换和投递渠道不作为必装项；文档格式依赖按下文处理，外部投递只检查渠道可用性，不为部署验证发送消息。现有安装与权限边界不因依赖检查而扩大。
