...
| `desktop-client-automation` | 使用 Cua Driver 或宿主原生 Computer Use 探索 Windows、macOS 桌面客户端及显式系统桌面；可按新鲜桌面截图动态点击主显示器图标，并把确认后的 Windows 操作路径固化为 Airtest 回放。 | 桌面客户端操作、桌面图标操作、跨平台探索、Windows 自动化回放 |
